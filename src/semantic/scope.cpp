...
Ref<Scope> PersScopeView::parent() {
    return scope()->parent();
}

Ref<const Scope> PersScopeView::parent() const {
    return scope()->parent();
}

void PersScopeView::for_each(ItemCb cb) {
    scope()->for_each(cb, _version);
}

ScopeFlags PersScopeView::flags() const {
    return scope()->flags();
}
