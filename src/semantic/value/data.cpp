...
DataView::DataView(
    ...
    assert(
        _view_type == type ||
        (_type->canon()->is_object() && _view_type->canon()->is_object() &&
         _view_type->canon()->is_impl_castable_to(_type->canon())));

    if (_atom_off == NoBitsize && _view_type->canon()->is_atom())
        _atom_off = off;
}
...
RValue DataView::load() const {
    auto rval = _type->canon()->load(_storage->bits(), _off);
    if (!_view_type->is_same(_type))
        rval = _type->canon()->cast_to(_view_type->canon(), std::move(rval));
    return rval;
}
...
DataView DataView::array_item(array_idx_t idx) {
    assert(is_array());
    auto array = _view_type->non_alias()->as_array();
...
DataView DataView::prop(Ref<Prop> prop_) {
    Ref<Class> cls{};
    auto type = _type->canon();
    auto view_type = _view_type->canon();
    if (type->is_class()) {
        cls = type->as_class();
        assert(view_type->is_class());
        assert(cls->is_same_or_base_of(view_type->as_class()));
    } else {
        assert(_type->is(AtomId));
        cls = _view_type->canon()->as_class();
    }
...
bool DataView::is_array() const { return _view_type->canon()->is_array(); }

bool DataView::is_object() const { return _view_type->canon()->is_object(); }

bool DataView::is_class() const { return _view_type->canon()->is_class(); }
