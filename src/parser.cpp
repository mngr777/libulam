...
void Parser::parse_module_var_or_type_def(Ref<ast::ModuleDef> node) {
    ...
    if (_tok.is(tok::Local)) {
        ...
        ulam_assert(_tok.in(tok::Constant, tok::TypeIdent));
...
Ptr<ast::TypeDef> Parser::parse_type_def() {
    ulam_assert(_tok.is(tok::Typedef));
    consume();
...
Parser::parse_fun_def_rest(Ptr<ast::FunRetType>&& ret_type, ast::Str name) {
    ...
    if (_tok.is(tok::BraceL)) {
        ...
        ulam_assert(_tok.is(tok::Semicol));
        consume();
    }
...
Ptr<ast::FunDef> Parser::parse_op_fun_def_rest(
    ...
    ulam_assert(tok::is_overloadable_op(op_tok_type));
...
Ptr<ast::WhichCase> Parser::parse_which_case(bool is_as_cond) {
    ulam_assert(_tok.in(tok::Case, tok::Otherwise));
...
Ptr<ast::Expr> Parser::parse_expr_climb_rest(
    ...
    while (true) {
        ...
        case Op::MemberAccess:
            lhs = parse_member_access_or_type_op_or_op_call(std::move(lhs));
            break;
        case Op::Ternary:
...
Ptr<ast::TypeOpExpr> Parser::parse_type_op_rest(
    ...
    ulam_assert((bool)type != (bool)expr);
    ulam_assert(_tok.is_type_op());
...
Ptr<ast::FunCall> Parser::parse_op_call() {
    ulam_assert(_tok.is(tok::Operator));
    // operator
    consume();
...
Parser::parse_member_access_or_type_op_or_op_call(Ptr<ast::Expr>&& obj) {
    ...
    if (_tok.is(tok::TypeIdent)) {
        // obj.Base1.Base2(p1, p2).<...>
        base_type = parse_base_type_select();
        if (!base_type)
            return {};
    }
    ...
    case tok::Ident:
        ...
        return parse_member_access_rest(
            op_loc_id, std::move(obj), std::move(base_type));
    case tok::Operator:
        // obj.Base1.Base2(p1, p2)[classid].operator<op>
        return parse_op_call_rest(
            op_loc_id, std::move(obj), std::move(base_type));
    default:
        if (_tok.is_type_op())
            return parse_type_op_rest({}, std::move(obj), std::move(base_type));
        unexpected();
        return {};
    }
}

Ptr<ast::MemberAccess> Parser::parse_member_access_rest(
    loc_id_t op_loc_id,
    Ptr<ast::Expr>&& obj,
    Ptr<ast::BaseTypeSelect>&& base_type) {
...
Ptr<ast::FunCall> Parser::parse_op_call_rest(
    loc_id_t op_loc_id,
    Ptr<ast::Expr>&& obj,
    Ptr<ast::BaseTypeSelect>&& base_type) {
    ...
    // NOTE: cannot determine op before parsing funcall:
...
Ptr<ast::BaseTypeSelect> Parser::parse_base_type_select() {
    ...
    do {
        ...
        if (!type_spec)
            return {};
        base_type->add(std::move(type_spec));

        // .
        consume_if(tok::Period);
    } while (_tok.is(tok::TypeIdent));
    ...
    if (_tok.is(tok::BracketL)) {
        ...
        auto expr = parse_expr();
        if (expr) {
            base_type->replace_classid(std::move(expr));
        } else {
            panic(tok::BracketR);
        }
        expect(tok::BracketR);
...
Ptr<ast::Ident> Parser::parse_ident(ident_flags_t flags) {
    ulam_assert(_tok.in(tok::Ident, tok::Local));
